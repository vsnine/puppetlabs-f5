...
Puppet::Type.type(:f5_key).provide(:f5_key, :parent => Puppet::Provider::F5) do
  ...
  def content
    message = { mode: resource[:mode], key_ids: {item: resource[:name]}}
  ...
  def content=(value)
    ...
    if resource[:real_content].match(/([-| ]*BEGIN CERTIFICATE[-| ]*.*?[-| ]*END CERTIFICATE[-| ]*)/m)
      ...
      message = { mode: resource[:mode], key_ids: { item: resource[:name] }, pem_data: { item: resource[:real_content] }, overwrite: true }
